...
A secret token that authenticates requests to the platform's API. Keys are scoped to a workspace and inherit the permissions of whoever created them. See [Permissions](/kasula-docs/documentation/core-concepts/permissions.md).
...
A piece of code that runs in response to a trigger — for example, a deploy completing or a comment being posted. See [Automations](/kasula-docs/documentation/guides/automations.md).
...
Your own domain (like `docs.yourcompany.com`) attached to a project, in place of the auto-generated subdomain. See [Custom domains](/kasula-docs/documentation/guides/custom-domains.md).
...
A workspace role that can view published deploys but nothing else. Guests don't count towards your member limit. See [Permissions](/kasula-docs/documentation/core-concepts/permissions.md).
...
A deployable unit. Each project has its own source, builds, deploys, environment, and domains. Projects are isolated from each other within a workspace. See [Workspaces and projects](/kasula-docs/documentation/core-concepts/workspaces-and-projects.md).
...
A workspace-level designation that determines what a member can do. Roles are: Guest, Viewer, Reviewer, Editor, and Admin. See [Permissions](/kasula-docs/documentation/core-concepts/permissions.md).
...
The event that starts an automation — for example, `deploy.succeeded` or `webhook.received`. See [Automations](/kasula-docs/documentation/guides/automations.md).
...
The top-level container for a team's work. Owns billing, members, and projects. See [Workspaces and projects](/kasula-docs/documentation/core-concepts/workspaces-and-projects.md).
